...
#         [self.time_projection[i].weight.data.fill_(0.0) for i in range(num_nodes)]
        ...
        # self.projections = nn.ModuleList([nn.GRUCell(input_size=1,
        #                             hidden_size=c_in) for i in range(num_nodes)])
        
        # for gru in self.projections:
        #     torch.nn.init.uniform_(gru.weight_ih, -0.001 / c_in, 0.001 / c_in)
        #     torch.nn.init.uniform_(gru.weight_hh, -0.001 / c_in, 0.001 / c_in)
...
#             p.data.fill_(0)
    ...
    def forward(self, t, node_feats, adj_matrix, x_obs):
        ...
        """
        batch_size, num_nodes = node_feats.size(0), self.num_nodes
        ...
        new_node_feats = []
        t_proj = self.time_projection(t.view(-1,1))
        for i in range(num_nodes):
            # t_scale = self.time_scale(t.view(-1, 1)).unsqueeze(1)
            # node_feats_normed = self.actnorms(node_feats[:,i,:]+t_proj)
            node_feats_normed = node_feats[:,i,:] / num_nodes
            ### change in 9.26 1.23 A.M.
            # node_feats_normed = F.normalize(node_feats_normed,dim=1)
        ...
        node_feats = torch.bmm(adj_matrix[:,:self.num_nodes, :self.num_nodes], node_feats_)
