...
class ConcatLinear_v2(nn.Module):
    ...
    def forward(self, t, x):
        features = self._layer(x)
        time_features = self._hyper_bias(t.view(-1, 1)).unsqueeze(1)
        return features + time_features
    
class NodesBias(nn.Module):
    def __init__(self, num_nodes, max_nodes, dim_out):
        super(NodesBias, self).__init__()
        self.D = num_nodes
        self.bias = nn.Parameter(torch.Tensor(max_nodes,dim_out))
        self.bias.data.fill_(0)
        
    def forward(self,x):
        """x: shape : [B,D,H]"""
        return x + self.bias[:self.D,:]
